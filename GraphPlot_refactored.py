...
def turning_points_from_derivative(deriv1: np.ndarray) -> pd.DataFrame:
    """
    Replicates original zero-crossing turning point detection.
    Output DataFrame indexed by integer positions with values 'Min'/'Max'.
    """
    d01 = pd.DataFrame({"Deriv 01": deriv1.ravel()})
    prev = 0
    pts = pd.DataFrame(np.nan, index=[0], columns=["Local"])
    for idx in d01.index[1:]:
        if (d01.iloc[idx]["Deriv 01"] > 0) and (d01.iloc[prev]["Deriv 01"] < 0):
            pts.loc[idx] = "Min"
        if (d01.iloc[idx]["Deriv 01"] < 0) and (d01.iloc[prev]["Deriv 01"] > 0):
            pts.loc[idx] = "Max"
        prev = idx
    pts = pts.drop(index=[0], errors="ignore")
    return pts
...
def compute_support_resistance_lines(
    ...
    """
    Mirrors original support/resistance construction:
    - Collect lows & highs between turning points
    - For each pair (j,curr) fit line and extend from start to end
    Returns lists of DataFrames (each with one column: 'Low'/'High').
    """
    col = price_df.columns[0]
    lin_reg = LinearRegression()

    if len(turning_pts.index) <= 1:
        return [], []

    p = turning_pts.index[0]
    lows_list: list[float] = []
    highs_list: list[float] = []

    sign = 1
    if turning_pts.loc[p].iloc[0] == "Max":
        sign = -1
        lows_list.append(float((price_df.loc[:p, col] * sign).max() * sign))
    else:
        highs_list.append(float((price_df.loc[:p, col] * sign).max() * sign))

    p0 = 0
    for i in turning_pts.index[:-1]:
        n = turning_pts.index[turning_pts.index.values.tolist().index(i) + 1]
        sign = 1
        if turning_pts.loc[i].iloc[0] == "Min":
            sign = -1
            lows_list.append(float((price_df.loc[p0:n, col] * sign).max() * sign))
        else:
            highs_list.append(float((price_df.loc[p0:n, col] * sign).max() * sign))
        p0 = i

    sign = 1
    p_last = turning_pts.index[-2]
    if turning_pts.iloc[-1].iloc[0] == "Max":
        highs_list.append(float((price_df.loc[p_last:, col] * sign).max() * sign))
    else:
        sign = -1
        lows_list.append(float((price_df.loc[p_last:, col] * sign).max() * sign))
    ...
    for idx in lows_df.index:
        lows_df.loc[idx, "Positions"] = price_df[price_df[col] == lows_list[idx]].index[0]
    for idx in highs_df.index:
        highs_df.loc[idx, "Positions"] = price_df[price_df[col] == highs_list[idx]].index[0]

    support_lines: list[pd.DataFrame] = []
    resistance_lines: list[pd.DataFrame] = []

    pmin = float(price_df.min().iloc[0]) * min_limit
    pmax = float(price_df.max().iloc[0]) * max_limit

    if len(lows_df.index) > 1:
        for j in np.arange(lows_df.shape[0] - 1):
            base_x = float(lows_df.loc[j, "Positions"])
            base_y = float(lows_df.loc[j, "Levels"])
            for curr in lows_df.index[(j + 1):]:
                lx = np.array([base_x, float(lows_df.loc[curr, "Positions"])]).reshape(-1, 1)
                ly = np.array([base_y, float(lows_df.loc[curr, "Levels"])])
                low_fit = lin_reg.fit(lx, ly)
                lx2 = np.array([price_df.index[0], base_x, float(lows_df.loc[curr, "Positions"]), price_df.index[-1]]).reshape(-1, 1)
                low_df = pd.DataFrame(low_fit.predict(lx2), index=[price_df.index[0], base_x, float(lows_df.loc[curr, "Positions"]), price_df.index[-1]], columns=["Low"])
                support_lines.append(low_df.clip(pmin, pmax, axis=1))

    if len(highs_df.index) > 1:
        for j in np.arange(highs_df.shape[0] - 1):
            base_x = float(highs_df.loc[j, "Positions"])
            base_y = float(highs_df.loc[j, "Levels"])
            for curr in highs_df.index[(j + 1):]:
                lx = np.array([base_x, float(highs_df.loc[curr, "Positions"])]).reshape(-1, 1)
                ly = np.array([base_y, float(highs_df.loc[curr, "Levels"])])
                high_fit = lin_reg.fit(lx, ly)
                lx2 = np.array([price_df.index[0], base_x, float(highs_df.loc[curr, "Positions"]), price_df.index[-1]]).reshape(-1, 1)
                high_df = pd.DataFrame(high_fit.predict(lx2), index=[price_df.index[0], base_x, float(highs_df.loc[curr, "Positions"]), price_df.index[-1]], columns=["High"])
                resistance_lines.append(high_df.clip(pmin, pmax, axis=1))
