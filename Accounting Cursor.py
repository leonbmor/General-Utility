...
# -------- Refactor helpers (non-breaking) --------
from dataclasses import dataclass
from pathlib import Path

def _resolve_path(*candidates: str) -> str:
    """Return the first existing path among candidates, else the last candidate."""
    for p in candidates:
        if not p:
            continue
        try:
...
def _read_csv_smart(local_name: str, fallback_path: str, **kwargs):
    """Prefer a local file (CWD or alongside this file); fallback to original path."""
    base = Path(__file__).resolve().parent if '__file__' in globals() else Path.cwd()
    local_candidates = [str(Path.cwd()/local_name), str(base/local_name)]
    path = _resolve_path(*local_candidates, fallback_path)
    return pd.read_csv(path, **kwargs)

@dataclass(frozen=True)
class RunFlags:
    realizeFX: bool = False
    ExpAsUgo: bool = True
    fifo: bool = True
    optSTR: bool = False
    accountingEUR: bool = True
    bundleALL: bool = False
    dropLAST: bool = False
    adjCDI: bool = False
...
def taxableAVG(taxable_df, mult, accMethod):
...
def taxableFIFO(taxable_df, accMethod):
...
def FXconvUSD(base_l, y_list, sEURFX_df):
...
def FXconvEUR(base_l, y_list, sEURFX_df):
...
def run(flags: RunFlags = RunFlags()):
    ...
    adjCDI = flags.adjCDI
    bundleALL = flags.bundleALL
    dropLAST = flags.dropLAST
    realizeFX = flags.realizeFX
    ExpAsUgo = flags.ExpAsUgo

    accountingEUR = flags.accountingEUR
    optSTR = flags.optSTR
    fifo = flags.fifo
    ...
    inception_cs = _read_csv_smart('96923_inception.csv', r"C:\Users\Utilizador\OneDrive\Documentos\Malta\LJJ\96923_inception.csv")
    all_cs = _read_csv_smart('96923_ALL.csv', r"C:\Users\Utilizador\OneDrive\Documentos\Malta\LJJ\96923_ALL.csv")
    all_saxo = _read_csv_smart('ALL_SAXO.csv', r"C:\Users\Utilizador\OneDrive\Documentos\Malta\LJJ\ALL_SAXO.csv")
    opts = _read_csv_smart('96923_ALL_EQUITIES.csv', r"C:\Users\Utilizador\OneDrive\Documentos\Malta\LJJ\96923_ALL_EQUITIES.csv")
    ...
    try:
        ...
        ubs_usd = _read_csv_smart('UBS_USD_ALL.csv', r"C:\Users\Utilizador\OneDrive\Documentos\Malta\LJJ\UBS_USD_ALL.csv")
    ...
    try:
        ubs_aud = _read_csv_smart('UBS_AUD_ALL.csv', r"C:\Users\Utilizador\OneDrive\Documentos\Malta\LJJ\UBS_AUD_ALL.csv") 
        for c_kw in fwdNF_l:
    ...
    try:
        ...
        ubs_eur = _read_csv_smart('UBS_EUR_ALL.csv', r"C:\Users\Utilizador\OneDrive\Documentos\Malta\LJJ\UBS_EUR_ALL.csv") 
    ...
    try:
        ubs_mxn = _read_csv_smart('UBS_MXN_ALL.csv', r"C:\Users\Utilizador\OneDrive\Documentos\Malta\LJJ\UBS_MXN_ALL.csv") 
        for c_kw in fwdNF_l:
    ...
    try:
        ubs_chf = _read_csv_smart('UBS_CHF_ALL.csv', r"C:\Users\Utilizador\OneDrive\Documentos\Malta\LJJ\UBS_CHF_ALL.csv") 
        for c_kw in fwdNF_l:
    ...
    expenses_l = FXconvEUR(expensesEUR_l, year_list, sEURFX_df)
    eqoMTM_l = FXconvUSD(eqoAccMTM_l, year_list, sEURFX_df)
    fxMTM_l = FXconvUSD(fxAccMTM_l, year_list, sEURFX_df)
    fxoMTM_l = FXconvUSD(fxoAccMTM_l, year_list, sEURFX_df)
    swapPV_l = FXconvUSD(swapAccPV_l, year_list, sEURFX_df)
    ...
    if (fifo):
        Feqty_df = taxableFIFO(Feqty_df, accMethod)
        opts_df = taxableFIFO(opts_df, accMethod)    
    else:    
        Feqty_df = taxableAVG(Feqty_df, False, accMethod)
        opts_df = taxableAVG(opts_df, True, accMethod)
...
if __name__ == '__main__':
    ...
    all_tickers = _read_csv_smart('tickers_all.csv', r"C:\Users\Utilizador\OneDrive\Documentos\Malta\LJJ\tickers_all.csv")
